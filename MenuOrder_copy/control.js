...
function confirm() {
    ...
    // Get the table_id from local storage
    var table_id = document.getElementById('table_id').getAttribute('value');

    // Prepare data to send including table_id
    var dataToSend = {
        table_id: table_id,
        order_data: localStorage.getItem('data')
    };

    // Open a POST request to confirm_order.php
    xhr.open('POST', 'confirm_order.php', true);

    // Set header for sending data in JSON format
    xhr.setRequestHeader('Content-Type', 'application/json');
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            // Hide the overlay after response received
            document.getElementById("overlay").style.display = "none";
        }
    };

    // Send the data to confirm_order.php
